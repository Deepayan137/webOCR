...
def get_features(root,dir):
    ...
    for each_ImageFile in ImageFiles[87:101]:
        if ((each_ImageFile)).endswith('jpg'):
            subprocess.call('./j-layout' + ' ' +  each_ImageFile+' '+'-psm'+' '+str(6), shell=True)
            os.remove(each_ImageFile + '.blocks.txt')
            os.remove(each_ImageFile + '.words.txt')
        count+=1


        PageNo = each_ImageFile.split('-')[-1].split('.')[0]
        #print PageNo
        #print os.path.join(os.path.join(root, 'Siddhi_gt'), PageNo.split('/')[1] + '.tif.txt')
        #if os.path.exists(os.path.join(os.path.join(root,'Siddhi_gt'),PageNo.split('/')[1]+'.tif.txt')):
            #print 'hi'

        text_file = (each_ImageFile+'.lines.txt')

        #text_file = os.path.join(root, each_ImageFile + '.segmentation_plot_file.txt')
        ...
        featureFile = open(os.path.join(os.path.join(root,feature_folder),image_name)+ '.Features.txt', 'a')
        try:
            ...
            for i in range(len(data)):
                ...
                cv2.imwrite(ann_folder + '/' + image_name, image)
...
def look_up(path):
    ...
    char=[]
    new_lup=[]
    if os.path.exists(path+'Advaita_new.txt')==True:
        os.remove(path+'Advaita_new.txt')
    lookup = open(path+'Advaita_new.txt', 'w')
    lines = file(sys.argv[4]).readlines()
    ...
    for i in range(len(truth)):
        line = truth[i].split('\n')[0]
        char.append(line.split(' '))

    for j in range(len(char)):
        for c in range(len(char[j])):
            l = char[j][c]

            new_lup.append(l)

    new_lup = set(new_lup)
    new_lup = list(new_lup)


    for i in range(len(new_lup)):
        lookup.write('*' + new_lup[i] + '\n')
    lookup.close()
#look_up('/home/deepayan/CVIT_codes/webOCR/multiocr/')

def shuffle(path):

    train_featureFile = open('Siddhi_trainFile.txt', 'a')
    val_featureFile = open('Siddhi_valFile.txt','a')
    test_featureFile = open('Siddhi_testFile.txt','a')
    tags=[]
    truth=[]
    feat=[]
    a= []
    ...
    for j in range(train_fr,val_fr):
        sum+=1
